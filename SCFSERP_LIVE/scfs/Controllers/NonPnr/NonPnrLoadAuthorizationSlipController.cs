...
{
    [SessionExpire]
    public class NonPnrLoadAuthorizationSlipController : Controller
    {
        // GET: NonPnrLoadAuthorizationSlip
	    ...
	    [Authorize(Roles = "NonPnrLoadSlipIndex")]
        ...
        public JsonResult GetAjaxData(JQueryDataTableParamModel param)
        {
            ...
            using (var e = new SCFSERPEntities())
            {
                ...
                var data = e.pr_Search_NonPnr_LoadSlip(param.sSearch,
                ...
                var aaData = data.Select(d => new string[] { d.ASLMDATE.Value.ToString("dd/MM/yyyy"), d.ASLMDNO, d.CONTNRNO, d.CONTNRSDESC, d.CHANAME, d.BOENO, d.IGMNO, d.GPLNO, d.DISPSTATUS.ToString(), d.ASLMID.ToString() }).ToArray();
	...
	[Authorize(Roles = "NonPnrLoadSlipEdit")]
        public void Edit(int id)
        {
            var strPath = ConfigurationManager.AppSettings["BaseURL"];

            Response.Redirect("" + strPath + "/NonPnrLoadAuthorizationSlip/Form/" + id);

            //Response.Redirect("/NonPnrLoadAuthorizationSlip/Form/" + id);
        }
        [Authorize(Roles = "NonPnrLoadSlipCreate")]
        ...
        public void savedata(FormCollection F_Form)
        {
            ...
            if (ASLMID == 0)
            {
                ...
                authorizatioslipmaster.ASLMNO = Convert.ToInt32(Autonumber.autonum("AUTHORIZATIONSLIPMASTER", "ASLMNO", "COMPYID=" + Convert.ToInt32(Session["compyid"]) + " AND SDPTID=9 AND ASLMTYPE=1").ToString());
        ...
        [Authorize(Roles = "NonPnrLoadSlipPrint")]
        public void PrintView(int? id = 0)
        {
            ...
            if (TMPRPT_IDS == "Successfully Added")
            {
                ...
                cryRpt.Load(ConfigurationManager.AppSettings["Reporturl"] + "NonPnr_Load_Slip.RPT");
        ...
        [Authorize(Roles = "NonPnrLoadSlipPrint")]
        public void TPrintView(int? id = 0)/*truck in*/
        {

            //  ........delete TMPRPT...//
            context.Database.ExecuteSqlCommand("DELETE FROM TMPRPT_IDS WHERE KUSRID='" + Session["CUSRID"] + "'");
            var TMPRPT_IDS = TMP_InsertPrint.InsertToTMP("TMPRPT_IDS", "Imp_Load_truckin", Convert.ToInt32(id), Session["CUSRID"].ToString());
            if (TMPRPT_IDS == "Successfully Added")
            {
                ReportDocument cryRpt = new ReportDocument();
                TableLogOnInfos crtableLogoninfos = new TableLogOnInfos();
        ...
        [Authorize(Roles = "NonPnrLoadSlipCancel")]
        ...
        public void Cancel()
        {
            ...
            if (temp.Equals("PROCEED"))
            {
                var sql = (from r in context.authorizatioslipmaster.Where(x => x.ASLMID == aslmid)
                           ...
                           join y in context.gateindetails on x.GIDID equals y.GIDID
                           //join y in context.opensheetdetails on x.OSDID equals y.OSDID
                           //join m in context.opensheetmasters on y.OSMID equals m.OSMID
                           select new { r.ASLMID, r.ASLMDNO, r.ASLMDATE, r.ASLMTIME, r.ASLMNO, x.DRVNAME, x.VHLNO, 
                           y.IGMNO, y.GPLNO
                           //m.OSMIGMNO, m.OSMLNO 
                           }).ToList();
                ...
                tab.PRDTDESC = "-";
                tab.UNITID = 0;
                tab.GPLNO = Convert.ToString(sql[0].GPLNO);
                tab.GPWGHT = 0;
                tab.GPEAMT = 0;
                tab.GPAAMT = 0;
                tab.IGMNO = Convert.ToString(sql[0].IGMNO);
                tab.GIISOCODE = "-";
                tab.GIDMGDESC = "-";
                tab.GPWTYPE = 0;
                tab.GPSTYPE = 0;
        ...
        [Authorize(Roles = "NonPnrLoadSlipDelete")]
        public void Del()
        {
            String id = Request.Form.Get("id");
            String fld = Request.Form.Get("fld");
        ...
        public ActionResult GateinMaxDate(string id)
        {
            ...
            var result = (from n in context.gateindetails
                          where n.IGMNO == igmno && n.GPLNO == gplno && n.SDPTID == 9

...
{
    [SessionExpire]
    public class BondInvoiceController : Controller
    {
        // GET: BondInvoice
        ...
        //[Authorize(Roles = "BondInvoiceEdit")]
        public void Edit(int id)
        {
            Response.Redirect("/BondInvoice/GSTForm/" + id);
        }


        //[Authorize(Roles = "BondInvoiceCreate")]
        public ActionResult GSTForm(int? id = 0)
        {
            ...
            if (id != 0)//....Edit Mode
            {
                ...
                vm.bondinvcdata = context.Database.SqlQuery<pr_Bond_Invoice_BondNO_Grid_Assgn_Result>("exec pr_Bond_Invoice_BondNO_Grid_Assgn @userid = '" + cusrid + "', @PBNDID=" + tab.TRANLMID + ", @PTRANMID=" + id + "").ToList();//........procedure  for edit mode details data
        ...
        public JsonResult GetAjaxData(JQueryDataTableParamModel param)
        {
            using (var e = new BondEntities())
            {
                ...
                var data = e.pr_Search_BondInvoice(param.sSearch, Convert.ToInt32(Request["iSortCol_0"]), Request["sSortDir_0"], param.iDisplayStart, param.iDisplayStart + param.iDisplayLength,
        ...
        public string Detail(string ids)
        {
            ...
            else
            {
                ...
                string cusrid = Session["CUSRID"].ToString();
                string gqry = "EXEC [pr_Bond_Invoice_BondNO_Grid_Assgn] @userid = '" + cusrid + "', @PBNDID =" + BNDID + ",";
                ...
                var query = context.Database.SqlQuery<pr_Bond_Invoice_BondNO_Grid_Assgn_Result>(gqry).ToList();
        ...
        public JsonResult RATECARD(string id)
        {
            if(id != null)
            {
                ...
                {
                    string tqry = "select SLABAMT,SLABMIN,SLABMAX from VW_BOND_RATECARDMASTER_FLX_ASSGN (NOLOCK) where TARIFFMID=" + TARIFFMID + " and SLABTID=" + SLABTID + " and HTYPE=" + HTYPE + " and (HANDTYPE= 2 Or HANDTYPE= 0) " + " AND YRDTYPE = " + YRDID + " AND PERIODTID = " + PRDTID + " and CHRGETYPE=" + CHRGETYPE + " and CONTNRSID= " + CONTNRSID + " and (PRDTGID = " + PRDTGID + " or PRDTGID =1) order by SLABMIN";
                    var query = context.Database.SqlQuery<VW_BOND_RATECARDMASTER_FLX_ASSGN>(tqry).ToList();
            ...
            using (BondContext dataContext = new BondContext())
            {
                using (var trans = dataContext.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        bondtransactionmaster.TRANTID = 2;
                        ...
                        if (TRANMID == 0)
                        {
                            ...
                            switch (regsid)
                            {
                                case 16: billformat = taxtype+"/ADB/"; break;
                                case 17: billformat = taxtype + "/ADB/"; break;
                                case 18: billformat = taxtype + "ZB/ADB/"; break;
        ...
        public void PrintView(int? id = 0)
        {
            ...
            //  ........delete TMPRPT...//
            context.Database.ExecuteSqlCommand("DELETE FROM NEW_TMPRPT_IDS WHERE  OPTNSTR='BONDINVOICE' and (KUSRID ='" + Session["CUSRID"] + "' or RPTID = " + Convert.ToInt32(id) + ")");
            //context.Database.ExecuteSqlCommand("DELETE FROM TMPRPT_IDS");
            var TMPRPT_IDS = TMP_InsertPrint.In_VT_Bond_NewInsertToTMP("NEW_TMPRPT_IDS", "BONDINVOICE", Convert.ToInt32(id), Session["CUSRID"].ToString());
            if (TMPRPT_IDS == "Successfully Added")
            {
                ...
                hstr = hstr + " and {VW_BOND_INVOICE_TRANSACTION_GST_PRINT_ASSGN.OPTNSTR} = 'BONDINVOICE'";
        ...
        //[Authorize(Roles = "BondInvoiceDelete")]
        public void Del()
        {

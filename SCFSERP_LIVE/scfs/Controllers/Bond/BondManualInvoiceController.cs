...
{
    [SessionExpire]
    public class BondManualInvoiceController : Controller
    {
        // GET: BondManualInvoice
        ...
        //[Authorize(Roles = "BondManualInvoiceEdit")]
        public void Edit(int id)
        {
            Response.Redirect("/BondManualInvoice/GSTForm/" + id);
        }


        //[Authorize(Roles = "BondManualInvoiceCreate")]
        public ActionResult GSTForm(int? id = 0)
        {
            ...
            if (id != 0)//....Edit Mode
            {
                ...
                string invdtlqry = "exec pr_Bond_Manual_Invoice_BondNO_Grid_Assgn @userid = '" + cusrid + "', @PBNDID=" + tab.TRANLMID + ", @PTRANMID=" + id + "";
                vm.bondmanualinvcdata = context.Database.SqlQuery<pr_Bond_Manual_Invoice_BondNO_Grid_Assgn_Result>(invdtlqry).ToList();//........procedure  for edit mode details data
        ...
        public JsonResult GetAjaxData(JQueryDataTableParamModel param)
        {
            using (var e = new BondEntities())
            {
                ...
                var data = e.pr_Search_BondManualInvoice(param.sSearch, Convert.ToInt32(Request["iSortCol_0"]), Request["sSortDir_0"], param.iDisplayStart, param.iDisplayStart + param.iDisplayLength,
        ...
        public string Detail(string ids)
        {
            ...
            else
            {
                ...
                var bildesc = "";
                if (Convert.ToString(param[19]) != "")
                    bildesc = Convert.ToString(param[19]);
                ...
                string cusrid = Session["CUSRID"].ToString();
                string gqry = "EXEC [pr_Bond_Manual_Invoice_BondNO_Grid_Assgn] @userid = '" + cusrid + "', @PBNDID =" + BNDID + ",";
                ...
                gqry = gqry + " @PBILLDESC = '" + bildesc + "'";
                var query = context.Database.SqlQuery<pr_Bond_Manual_Invoice_BondNO_Grid_Assgn_Result >(gqry).ToList();
                ...
                tabl = tabl + "<th>Bond No</th><th class='hide'>Tariff</th><th class='hide'>Slab Type</th> <th>Bill Description</th> <th class='hide'>Period</th><th>BType</th><th>CSize</th><th>Wks./Cntnrs./Ins.</th><th>Oprn</th><th>NOP</th><th>Charge Date</th><th>Rate</th><th>Amount</th>";
                ...
                foreach (var rslt in query)
                {
                    ...
                    tabl = tabl + "<td class='hide'><input type=text id=TARIFFMID value='" + rslt.TARIFFMID + "' class='hide TARIFFMID' readonly name=TARIFFMID style='width:40px'><input type=text id=TARIFFDESC  readonly value='" + rslt.TARIFFDESC + "' class=TARIFFDESC name=TARIFFDESC style='width:120px'></td>";
                    tabl = tabl + "<td class='hide'><input type=text id=SLABTID value='" + rslt.SLABTID + "' class='hide SLABTID'  readonly name=SLABTID style='width:40px'><input type=text id=SLABTDESC value='" + rslt.SLABTDESC + "' class=SLABTDESC  readonly name=SLABTDESC style='width:120px'></td>";
                    ...
                    tabl = tabl + "<td><textarea style='width:450px;' class='form-control  BILLDESC' cols='8' id='BILLDESC' name='BILLDESC' rows='3'>" + rslt.BILLDESC + "</textarea></td>";
            ...
            using (BondContext dataContext = new BondContext())
            {
                using (var trans = dataContext.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        bondtransactionmaster.TRANTID = 4;
                        ...
                        if (TRANMID == 0)
                        {
                            ...
                            switch (regsid)
                            {
                                case 16: billformat = taxtype + "/MNUL/INB/CU/"; break;
                                case 17: billformat = taxtype + "/MNUL/INB/CH/"; break;
                                case 18: billformat = taxtype + "ZB/MNUL/INB/"; break;
                        ...
                        string[] BILLDESC = F_Form.GetValues("BILLDESC");
                        ...
                        for (int count = 0; count < boolTRANDIDS.Count(); count++)
                        {
                            if (boolTRANDIDS[count] == "true")
                            {
                                ...
                                bondtransactiondetail.TRANDEBNDNO = "-";
                                bondtransactiondetail.EBNDID = 0;
                                bondtransactiondetail.TRANHTYPE = 0;
                                bondtransactiondetail.TRANDWGHT = 0;
                                bondtransactiondetail.BILLDESC = (BILLDESC[count]).ToString();
                                ...
                                bondtransactiondetail.TRANHTYPE = 0;// Convert.ToInt16(TRANHTYPE[count]);
        ...
        public void PrintView(int? id = 0)
        {
            ...
            var TMPRPT_IDS = TMP_InsertPrint.In_VT_Bond_NewInsertToTMP("NEW_TMPRPT_IDS", "BONDMANUALINVOICE", Convert.ToInt32(id), Session["CUSRID"].ToString());
            if (TMPRPT_IDS == "Successfully Added")
            {
                ...
                hstr = hstr + " and {VW_BOND_INVOICE_TRANSACTION_GST_PRINT_ASSGN.OPTNSTR} = 'BONDMANUALINVOICE'";
                ...
                string path = pdfPath + "\\" + Session["CUSRID"] + "\\BondInvoice";
        ...
        #region Delete Bond Invoice
        //[Authorize(Roles = "BondManualInvoiceDelete")]
        public void Del()
        {

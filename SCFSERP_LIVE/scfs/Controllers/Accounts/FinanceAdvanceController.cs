...
{
    [SessionExpire]
    public class FinanceAdvanceController : Controller
    {       
        // GET: FinanceAdvance
        ...
        #region Finance Advane Index
        [Authorize(Roles = "FinanceAdvanceIndex")]
        ...
        public JsonResult GetAjaxData(JQueryDataTableParamModel param)
        {
            using (var e = new SCFSERPEntities())
            {
                ...
                var data = e.pr_Search_Advance(param.sSearch, Convert.ToInt32(Request["iSortCol_0"]), Request["sSortDir_0"], param.iDisplayStart, param.iDisplayStart + param.iDisplayLength,
                    totalRowsCount, filteredRowsCount, Convert.ToInt32(Session["compyid"]), Convert.ToInt32(Session["SDPTID"]), 62, Convert.ToDateTime(Session["SDATE"]), Convert.ToDateTime(Session["EDATE"]));
        ...
        [Authorize(Roles = "FinanceAdvanceEdit")]
        public void Edit(int id)
        {
            Response.Redirect("/FinanceAdvance/AForm/" + id);
        }
        #endregion

        #region Advance Form
        [Authorize(Roles = "FinanceAdvanceCreate")]
                ...
                using (var trans = context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (TRANMID == 0)
                        {
                            ...
                            //transactionmaster.TRANNO = Convert.ToInt16(Autonumber.autonum("transactionmaster", "TRANNO", "REGSTRID=62 and COMPYID=" + Convert.ToInt32(Session["compyid"]) + "and SDPTID=" + Convert.ToInt32(SDPID) + "and TRANBTYPE=0").ToString());

                            //transactionmaster.TRANNO = Convert.ToInt16(Autonumber.autonum("transactionmaster", "TRANNO", "REGSTRID=62 and COMPYID=" + Convert.ToInt32(Session["compyid"]) + " and SDPTID=" + Convert.ToInt32(SDPTID) + " and TRANBTYPE=0").ToString());

                            transactionmaster.TRANNO = Convert.ToInt32(Autonumber.autonum("transactionmaster", "TRANNO", " REGSTRID=62 and COMPYID=" + Convert.ToInt32(Session["compyid"]) + " ").ToString());
        ...
        #region Print View
        [Authorize(Roles = "FinanceAdvancePrint")]
        public void PrintView(int? id = 0)
        {
            ...
            var TMPRPT_IDS = TMP_InsertPrint.InsertToTMP("TMPRPT_IDS", "ADVANCE", Convert.ToInt32(id), Session["CUSRID"].ToString());
            if (TMPRPT_IDS == "Successfully Added")
            {
                ...
                cryRpt.Load(ConfigurationManager.AppSettings["Reporturl"] + "Finance_Advance.RPT");
                cryRpt.RecordSelectionFormula = "{VW_FINANCE_ADVANCE_PRINT.KUSRID} = '" + Session["CUSRID"].ToString() + "' AND {VW_FINANCE_ADVANCE_PRINT.TRANMID} = " + id;
        ...
        [Authorize(Roles = "FinanceAdvanceDelete")]
        public void Del()
        {

...
{
    [SessionExpire]
    public class ExportAdvanceController : Controller
    {
        // GET: ExportAdvance
        ...
        #region Export Advance Index
        [Authorize(Roles = "ExportAdvanceIndex")]
        ...
        public JsonResult GetAjaxData(JQueryDataTableParamModel param)
        {
            using (var e = new SCFSERPEntities())
            {
                ...
                var data = e.pr_Search_Advance(param.sSearch, Convert.ToInt32(Request["iSortCol_0"]), Request["sSortDir_0"], param.iDisplayStart, param.iDisplayStart + param.iDisplayLength,
                    totalRowsCount, filteredRowsCount, Convert.ToInt32(Session["compyid"]), 2, 19, Convert.ToDateTime(Session["SDATE"]), Convert.ToDateTime(Session["EDATE"]));
        ...
        [Authorize(Roles = "ExportAdvanceEdit")]
        public void Edit(int id)
        {
            Response.Redirect("/ExportAdvance/EAForm/" + id);
        }
        #endregion

        #region Advance Form
        [Authorize(Roles = "ExportAdvanceCreate")]
        public ActionResult EAForm(int id = 0)
        {
            if (Convert.ToInt32(Session["compyid"]) == 0) { return RedirectToAction("Login", "Account"); }
                ...
                using (var trans = context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (TRANMID == 0)
                        {
                            ...
                            transactionmaster.TRANNO = Convert.ToInt32(Autonumber.autonum("transactionmaster", "TRANNO", "REGSTRID=19 and COMPYID=" + Convert.ToInt32(Session["compyid"]) + "and SDPTID=" + Convert.ToInt32(SDPID) + "and TRANBTYPE=0").ToString());
        ...
        #region Print View
        [Authorize(Roles = "ExportAdvancePrint")]
        public void PrintView(int? id = 0)
        {
            ...
            var TMPRPT_IDS = TMP_InsertPrint.InsertToTMP("TMPRPT_IDS", "EXPORTADVANCE", Convert.ToInt32(id), Session["CUSRID"].ToString());
            if (TMPRPT_IDS == "Successfully Added")
            {
                ...
                cryRpt.Load(ConfigurationManager.AppSettings["Reporturl"] + "Export_Advance.RPT");
                cryRpt.RecordSelectionFormula = "{VW_EXPORT_ADVANCE_PRINT.KUSRID} = '" + Session["CUSRID"].ToString() + "' AND {VW_EXPORT_ADVANCE_PRINT.TRANMID} = " + id;
        ...
        [Authorize(Roles = "ExportAdvanceDelete")]
        public void Del()
        {

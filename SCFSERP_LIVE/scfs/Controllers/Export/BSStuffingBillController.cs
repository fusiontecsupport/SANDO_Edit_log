...
{
    [SessionExpire]
    public class BSStuffingBillController : Controller
    {
        //
        // GET: /BSStuffingBill/
        ...
        [Authorize(Roles = "ExportBSStuffingBillIndex")]
        public ActionResult Index()
        {
            ...
            //............Billed to....//
            //ViewBag.REGSTRID = new SelectList(context.Export_Invoice_Register.Where(x => x.REGSTRID != 1).Where(x => x.REGSTRID != 2).Where(x => x.REGSTRID != 6).Where(x => x.REGSTRID != 16).Where(x => x.REGSTRID != 17).Where(x => x.REGSTRID != 18), "REGSTRID", "REGSTRDESC", Convert.ToInt32(Session["REGSTRID"]));
            ViewBag.REGSTRID = new SelectList(context.Export_Invoice_Register.Where(x => x.REGSTRID == 46 || x.REGSTRID == 47 || x.REGSTRID == 48), "REGSTRID", "REGSTRDESC");
            //.....end
        ...
        public JsonResult GetAjaxData(JQueryDataTableParamModel param)
        {
            using (var e = new CFSExportEntities())
            {
                ...
                var aaData = data.Select(d => new string[] { d.TRANDATE.Value.ToString("dd/MM/yyyy"), d.TRANTIME.Value.ToString("hh:mm tt"), d.TRANDNO.ToString(), d.TRANREFNAME, d.TRANNAMT.ToString(), d.NOC.ToString(), d.DISPSTATUS, d.GSTAMT.ToString(), d.TRANMID.ToString() }).ToArray();
                return Json(new
                {
                    sEcho = param.sEcho,
                    aaData = aaData,
        ...
        [Authorize(Roles = "ExportBSStuffingBillEdit")]
        public void Edit(int id)
        {
            Response.Redirect("/BSStuffingBill/GSTForm/" + id);
        }
        //......................Form data....................//
        [Authorize(Roles = "ExportBSStuffingBillCreate")]
        ...
        public ActionResult GSTForm(int id = 0)
        {
            ...
            //selectedItemBILLEDTO = new SelectListItem { Text = "ZERO BILL", Value = "18", Selected = false };
            //selectedBILLEDTO.Add(selectedItemBILLEDTO);
            //ViewBag.REGSTRID = new SelectList(context.Export_Invoice_Register.Where(x => x.REGSTRID != 1).Where(x => x.REGSTRID != 2).Where(x => x.REGSTRID != 6).Where(x => x.REGSTRID != 16).Where(x => x.REGSTRID != 17).Where(x => x.REGSTRID != 18), "REGSTRID", "REGSTRDESC");
            ViewBag.REGSTRID = new SelectList(context.Export_Invoice_Register.Where(x => x.REGSTRID == 46 || x.REGSTRID == 47 || x.REGSTRID == 48), "REGSTRID", "REGSTRDESC");
            //.....end
                    ...
                    try
                    {
                        ...
                        if (TRANMID == 0)
                        {
                            ...
                            else if (regsid == 46 && btype == 2)
                            {
                                ...
                                //ano = transactionmaster.TRANNO;
                                billformat = "GRT/BS/CUS/";
                                //prfx = string.Format(format + "{0:D5}", ano);
                                // transactionmaster.TRANDNO = prfx.ToString();
                            }

                            else if (regsid == 47 && btype == 1)
                            {
                                ...
                                //ano = transactionmaster.TRANNO;
                                billformat = "STL/BS/ST/CH/";
                                //prfx = string.Format(format + "{0:D5}", ano);
                                //transactionmaster.TRANDNO = prfx.ToString();
                            }

                            else if (regsid == 47 && btype == 2)
                            {
                                ...
                                //ano = transactionmaster.TRANNO;
                                billformat = "GRT/BS/CHA/";
                                //prfx = string.Format(format + "{0:D5}", ano);
                                //transactionmaster.TRANDNO = prfx.ToString();
                            }
                            else if (regsid == 48 && btype == 1)
                            {
                                ...
                                //ano = transactionmaster.TRANNO;
                                billformat = "ZB/BS/STUFF/";
                                //prfx = string.Format(format + "{0:D5}", ano);
                                //transactionmaster.TRANDNO = prfx.ToString();
                            }
                            else if (regsid == 48 && btype == 2)
                            {
                                ...
                                //ano = transactionmaster.TRANNO;
                                billformat = "ZB/BS/GRT/";
                                //prfx = string.Format(format + "{0:D5}", ano);
                                //transactionmaster.TRANDNO = prfx.ToString();
                            }
                            //........end of autonumber

                            transactionmaster.TRANNO = Convert.ToInt16(auto_numbr_invoice.gstexportautonum_BS("transactionmaster", "TRANNO", "0", Session["compyid"].ToString(), btype.ToString(), transactionmaster.TRANREFID).ToString());
        ...
        public string Detail(string ids)
        {
            ...
            var query = context.Database.SqlQuery<pr_Export_Invoice_Stuff_Flx_Assgn_Result>("EXEC pr_Export_Invoice_Stuff_Flx_Assgn @PSTFMID=" + STFMID + ",@PEDATE='" + edt + "',@PTRANMID=" + TRANMID).ToList();
            ...
            foreach (var rslt in query)
            {
                tabl = tabl + "<tbody><tr><td><input type=checkbox id=STFDIDS class=STFDIDS name=STFDIDS value='' " + count + "  onchange=total() style='width:30px'><input type=text id=boolSTFDIDS class='hide boolSTFDIDS' name=boolSTFDIDS value=''></td><td class=hide><input type=text id=STFDID value=" + rslt.STFDID + "  class=STFDID name=STFDID></td> <td><input type=text id=TRANDREFNO value=" + rslt.CONTNRNO + "  class=TRANDREFNO readonly='readonly' name=TRANDREFNO style='width:110px'></td><td><input type=text id=SIZE value='" + rslt.CONTNRSDESC + "' class=SIZE name=SIZE style='width:40px' readonly='readonly'></td><td ><input type=text id=TRANIDATE value='" + rslt.GIDATE + "' class=TRANIDATE name=TRANIDATE readonly='readonly'></td><td ><input type=text id=TRANSDATE value='" + rslt.SBDDATE + "' class=TRANSDATE name=TRANSDATE readonly='readonly'></td><td><input type=text id='TRANEDATE' value='" + rslt.GODATE + "' class='TRANEDATE datepicker' name='TRANEDATE' style='width:135px' onchange='calculation()'></td><td><input type=text id=TRANDSAMT value='0' class=TRANDSAMT name=TRANDSAMT style='width:75px' readonly='readonly'><input type=text id=TRAND_COVID_DISC_AMT value='0' class='TRAND_COVID_DISC_AMT hide' name=TRAND_COVID_DISC_AMT style='width:75px' readonly='readonly'></td><td><input type=text id=TRANDHAMT value='0' class=TRANDHAMT name=TRANDHAMT readonly=readonly style='width:75px'></td><td><input type=text value='0' id=TRANDEAMT class=TRANDEAMT name=TRANDEAMT style=width:45px readonly=readonly ></td><td><input type=text value='0' id=TRANDFAMT class=TRANDFAMT name=TRANDFAMT style=width:45px readonly=readonly ></td><td><input type=text id=TRANDPAMT value='0' class=TRANDPAMT name=TRANDPAMT  style='width:45px' readonly=readonly ></td><td><input type=text value='0' id=TRANDNAMT class=TRANDNAMT name=TRANDNAMT readonly='readonly' style=width:100px> <td class=hide><input type=text id=TRANDNOP class=TRANDNOP name=TRANDNOP value=" + rslt.STFDNOP + "><input type=text id=TRANDQTY value=" + rslt.STFDQTY + "  class=TRANDQTY name=TRANDQTY ></td>  <td class=hide><input type=text id=CONTNRTID value=0  class=CONTNRTID name=CONTNRTID ><input type=text id=CONTNRSID value=" + rslt.CONTNRSID + "  class=CONTNRSID name=CONTNRSID ><input type=text id=TRANDREFID value=" + rslt.GIDID + "  class=TRANDREFID name=TRANDREFID ></td><td class=hide><input type=text id=TRANDID value=0  class=TRANDID name=TRANDID ></td><td class='hide'><input type=text id=days value=0  class=days name=days ></td><td class=hide><input type=text id=RAMT1 value='0'  class=RAMT1 name=RAMT1 style='display:none1' ><input type=text id=RAMT2 value='0'  class=RAMT2 name=RAMT2 style='display:none1' ><input type=text id=RAMT3 value='0'  class=RAMT3 name=RAMT3 style='display:none1' ><input type=text id=RAMT4 value='0'  class=RAMT4 name=RAMT4 style='display:none1' ><input type=text id=RAMT5 value='0'  class=RAMT5 name=RAMT5 style='display:none1' ><input type=text id=RAMT6 value='0'  class=RAMT6 name=RAMT6 style='display:none1' ><input type=text id=RAMT7 value='0'  class=RAMT7 name=RAMT7 style='display:none1' ></td><td class=hide><input type=text id=SLABMIN value='0'  class=SLABMIN name=SLABMIN style='display:none1' ><input type=text id=SLABMAX value='0'  class=SLABMAX name=SLABMAX style='display:none1' ><input type=text id=SLABMIN1 value='0'  class=SLABMIN1 name=SLABMIN1 style='display:none1' ><input type=text id=SLABMAX1 value='0'  class=SLABMAX1 name=SLABMAX1 style='display:none1' ><input type=text id=SLABMIN2 value='0'  class=SLABMIN2 name=SLABMIN2 style='display:none1' ><input type=text id=SLABMAX2 value='0'  class=SLABMAX2 name=SLABMAX2 style='display:none1' > <input type=text id=SLABMIN3 value='0'  class=SLABMIN3 name=SLABMIN3 style='display:none1' ><input type=text id=SLABMAX3 value='0'  class=SLABMAX3 name=SLABMAX3 style='display:none1' >  <input type=text id=SLABMIN4 value='0'  class=SLABMIN4 name=SLABMIN4 style='display:none1' ><input type=text id=SLABMAX4 value='0'  class=SLABMAX4 name=SLABMAX4 style='display:none1' > <input type=text id=SLABMIN5 value='0'  class=SLABMIN5 name=SLABMIN5 style='display:none1' ><input type=text id=SLABMAX5 value='0'  class=SLABMAX5 name=SLABMAX5 style='display:none1' >  <input type=text id=SLABMIN6 value='0'  class=SLABMIN6 name=SLABMIN6 style='display:none1' ><input type=text id=SLABMAX6 value='0'  class=SLABMAX6 name=SLABMAX6 style='display:none1' > </td><td class=hide> <input type=text id=RCAMT1 value=0  class=RCAMT1 name=RCAMT1 style='display:none1' ><input type=text id=RCAMT2 value=0  class=RCAMT2 name=RCAMT2 style='display:none1' ><input type=text id=RCAMT3 value=0  class=RCAMT3 name=RCAMT3 style='display:none1'><input type=text id=RCAMT4 value='0'  class=RCAMT4 name=RCAMT4 style='display:none1' ><input type=text id=RCAMT5 value='0'  class=RCAMT5 name=RCAMT5 style='display:none1' ><input type=text id=RCAMT6 value='0'  class=RCAMT6 name=RCAMT6 style='display:none1' ><input type=text id=RCAMT7 value='0'  class=RCAMT7 name=RCAMT7 style='display:none1' ></td><td class=hide><input type=text id=RCOL1 value='0'  class=RCOL1 name=RCOL1 style='display:none1' ><input type=text id=RCOL2 value='0'  class=RCOL2 name=RCOL2 style='display:none1' ><input type=text id=RCOL3 value='0'  class=RCOL3 name=RCOL3 style='display:none1' ><input type=text id=RCOL4 value='0'  class=RCOL4 name=RCOL4 style='display:none1' ><input type=text id=RCOL5 value='0'  class=RCOL5 name=RCOL5 style='display:none1' ><input type=text id=RCOL6 value='0'  class=RCOL6 name=RCOL6 style='display:none1' ><input type=text id=RCOL7 value='0'  class=RCOL7 name=RCOL7 style='display:none1' ><input type='text' value=" + rslt.STATETYPE + " id='F_STATETYPE' class='F_STATETYPE' name='F_STATETYPE'></td></tr></tbody>";
                count++;
            }
            tabl = tabl + "</Table>";
        ...
        public JsonResult Bill_Detail(string id)
        {
            ...
            var EOPTID = Convert.ToInt32(param[4]);/* INSTEAD OF SLABTID=5 ,,PARAM[5]*/
            if (TARIFFMID == 4)
            {
        ...
        public JsonResult GSTDefaultCF()
        {
            DbSqlQuery<CostFactorMaster> data = context.costfactormasters.SqlQuery("select * from costfactormaster  where CFID  in(83) order by CFID DESC");
            //  DbSqlQuery<CostFactorMaster> data2 = context.costfactormasters.SqlQuery("select * from costfactormaster  where DISPSTATUS=0 and CFID  in(0) order by CFID");
        ...
        //..........................Printview...
        [Authorize(Roles = "ExportBSStuffingBillPrint")]
        public void PrintView(string id)
        {
            ...
            if (TMPRPT_IDS == "Successfully Added")
            {
                ...
                //        else
                //        { cryRpt.Load( ConfigurationManager.AppSettings["Reporturl"]+  "GST_BS_Export_Stuff_Invoice_Exp.RPT"); }
                ...
                //        else
                //        { cryRpt.Load( ConfigurationManager.AppSettings["Reporturl"]+  "GST_BS_Export_Stuff_Invoice.RPT"); }

                //        break;
                //}

                RptNamePath = ConfigurationManager.AppSettings["Reporturl"] + "GST_BS_Export_Stuff_Invoice.RPT";
        ...
        //end
        //...............Delete Row.............
        [Authorize(Roles = "ExportBSStuffingBillDelete")]
        public void Del()
        {

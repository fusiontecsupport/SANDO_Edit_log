...
{
    [SessionExpire]
    public class BSSealBillController : Controller
    {
        //
        // GET: /BSSealBill/
        ...
        [Authorize(Roles = "ExportBSSealBillIndex")]
        public ActionResult Index()
        {
            ...
            //............Billed to....//
            //ViewBag.REGSTRID = new SelectList(context.Export_Invoice_Register.Where(x => x.REGSTRID != 1).Where(x => x.REGSTRID != 2).Where(x => x.REGSTRID != 6).Where(x => x.REGSTRID != 16).Where(x => x.REGSTRID != 17).Where(x => x.REGSTRID != 18), "REGSTRID", "REGSTRDESC", Session["REGSTRID"]);
        ...
        [Authorize(Roles = "ExportBSSealBillEdit")]
        public void Edit(int id)
        {
            Response.Redirect("/BSSealBill/GSTForm/" + id);
        }
        [Authorize(Roles = "ExportBSSealBillCreate")]
                ...
                using (var trans = dataContext.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (TRANMID == 0)
                        {
                            ...
                            switch (regsid)
                            {
                                case 46: billformat = "STL/SL/CU/"; break;
                                case 47: billformat = "STL/SL/CH/"; break;
                                case 48: billformat = "ZB/SEAL/"; break;

                            }
                            string btyp= auto_numbr_invoice.GetCateBillType(Convert.ToInt32(transactionmaster.TRANREFID)).ToString();
                            if (btyp == "")
                            {
                                format = "BSEXP/" + Session["GPrxDesc"] + "/";
                            }
                             else
                                format ="BSEXP/" + Session["GPrxDesc"] + "/"+ btyp;
        ...
        public JsonResult Bill_Detail(string id)
        {
            ...
            {
                var query = context.Database.SqlQuery<VW_EXPORT_RATECARDMASTER_FLX_ASSGN>("select SLABTID,SLABAMT from VW_EXPORT_RATECARDMASTER_FLX_ASSGN where TARIFFMID=" + TARIFFMID + " and SLABTID in (6,7,14,15,16) and CHRGETYPE=" + CHRGETYPE + " and CONTNRSID= " + CONTNRSID + " and (EOPTID=" + EOPTID + " or (EOPTID=0 and SLABTID <> 6))").ToList();
                return Json(query, JsonRequestBehavior.AllowGet);
            }
        ...
        //..........................Printview...
        [Authorize(Roles = "ExportBSSealBillPrint")]
            ...
            if (TMPRPT_IDS == "Successfully Added")
            {
                ...
                switch (billedto)
                {
                    case 1:
                        ...
                        { cryRpt.Load( ConfigurationManager.AppSettings["Reporturl"]+  "GST_BS_Export_Seal_Invoice_Exp.RPT"); }
                    ...
                    default:
                        ...
                        { cryRpt.Load( ConfigurationManager.AppSettings["Reporturl"]+  "GST_BS_Export_Seal_Invoice.RPT"); }
        ...
        //end
        //...............Delete Row.............
        [Authorize(Roles = "ExportBSSealBillDelete")]
        public void Del()
        {

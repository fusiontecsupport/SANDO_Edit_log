...
{
    [SessionExpire]
    public class ExportEmptyVTController : Controller
    {
        // GET: ExportEmptyVT
        ...
        #region Index Page
        //[Authorize(Roles = "ExportEmptyVTIndex")]
        ...
        public JsonResult GetAjaxData(JQueryDataTableParamModel param)
        {
            using (var e = new CFSExportEntities())
            {
                ...
                var data = e.pr_Search_Export_Vehicle_Ticket_Master(param.sSearch, Convert.ToInt32(Request["iSortCol_0"]), Request["sSortDir_0"], param.iDisplayStart, param.iDisplayStart + param.iDisplayLength,
                    totalRowsCount, filteredRowsCount, Convert.ToInt32(Session["compyid"]), Convert.ToDateTime(Session["SDATE"]), Convert.ToDateTime(Session["EDATE"]), 5); //Convert.ToInt32(Session["VTCTYPE"])
        ...
        //[Authorize(Roles = "ExportEmptyVTEdit")]
        public void Edit(string id)
        {
            Response.Redirect("/ExportEmptyVT/Form/" + id);
        }
        #endregion
        ...
        {
            Response.Redirect("/ExportEmptyVT/DForm/" + id);
        }
        #endregion
        ...
        public ActionResult DForm(string id = "0")
        {
            ...
            if (VTDID != 0)//Edit Mode
            {
                ...
                var query1 = (from r in context.gateindetails.Where(x => x.SDPTID == 2 && x.DISPSTATUS == 0 && x.CONTNRID == 1)
                              ...
                              select new { r.GIDID, r.GIDATE, r.EXPRTRNAME, r.CHANAME, r.CHAID, r.CONTNRNO, r.STMRNAME, r.IGMNO, r.GPLNO, s.CONTNRSDESC, r.GPWGHT, r.CSEALNO, r.LPSEALNO }
                           ).ToList();
                if (query1.Count > 0)
                {
                    ViewBag.CONTNRNO = query1[0].CONTNRNO.ToString();
            ...
            using (context = new SCFSERPContext())
            {
                ...
                try
                {
                    using (var trans = context.Database.BeginTransaction())
                    {
                        ...
                        //..........................gateout detail...................//                                                
                        GateOutDetail ago = new GateOutDetail();
                        var query = context.Database.SqlQuery<GateOutDetail>("select * from GateOutDetail where gidid = " + tab.GIDID).ToList();
                        
                        if (query.Count == 0)
                        {
                            ago.GODATE = tab.VTDATE; ;
                            ago.GOTIME = tab.VTTIME; ;
                            ago.COMPYID = Convert.ToInt32(Session["compyid"]);
                            ago.VHLNO = tab.VHLNO;
                            ago.GIDID = Convert.ToInt32(Request.Form.Get("GIDID"));
                            ago.SDPTID = 2;
                            ago.REGSTRID = 1;
                            ago.TRANDID = 0;
                            ago.GOBTYPE = 1;

                            ago.CUSRID = Session["CUSRID"].ToString();
                            ago.LMUSRID = Session["CUSRID"].ToString();
                            ago.PRCSDATE = DateTime.Now;
                            ago.EHIDATE = DateTime.Now;
                            ago.EHITIME = DateTime.Now;
                            ago.GDRVNAME = tab.DRVNAME;
                            if ((ago.GODID).ToString() != "0")
                            {
                                context.Entry(ago).State = System.Data.Entity.EntityState.Modified;
                                context.SaveChanges();
                            }
                            else
                            {
                                ago.GONO = Convert.ToInt16(Autonumber.autonum("GateOutDetail", "GONO", "COMPYID=" + Convert.ToInt32(Session["compyid"]) + " and SDPTID=2").ToString());
                                int ano = ago.GONO;
                                // string prfx = string.Format("{0:D5}", ano);
                                ago.GODNO = ano.ToString();
                                context.gateoutdetail.Add(ago);
                                context.SaveChanges();
                            }
                        }
                        //..........................gateout detail...................//
            ...
            using (context = new SCFSERPContext())
            {
                ...
                try
                {
                    using (var trans = context.Database.BeginTransaction())
                    {
                        ...
                        {
                            tab.vtdetail.VTNO = Convert.ToInt32(Autonumber.autonum("vehicleticketDetail", "VTNO", "COMPYID=" + Convert.ToInt32(Session["compyid"]) + " and SDPTID=2 and VTCTYPE=5").ToString());
        ...
        {
            var query = context.Database.SqlQuery<VW_VEHICLETICKET_EXPORT_CONTAINER_CTRL_ASSGN>("select * from VW_VEHICLETICKET_EXPORT_CONTAINER_DIRECT_CTRL_ASSGN where GIDID=" + id).ToList();
            return Json(query, JsonRequestBehavior.AllowGet);
        }
        #endregion
        ...
        public JsonResult AutoContainer(string term)
        {
            ...
            string sqry1 = "select distinct CONTNRNO,GIDID  from VW_VEHICLETICKET_EXPORT_CONTAINER_CBX_ASSGN_DIRECT WHERE CONTNRNO LIKE '%" + term + "%'  ";

            var result = context.Database.SqlQuery<VW_VEHICLETICKET_EXPORT_CONTAINER_CBX_ASSGN_DIRECT>(sqry1).ToList();
        ...
        #region Printview
        //[Authorize(Roles = "ExportEmptyVTPrint")]
        public void PrintView(int id)
        {
            ...
            var TMPRPT_IDS = TMP_InsertPrint.InsertToTMP("TMPRPT_IDS", "ExportEmptyVT", Convert.ToInt32(id), Session["CUSRID"].ToString());
            if (TMPRPT_IDS == "Successfully Added")
            {
                ...
                cryRpt.Load(ConfigurationManager.AppSettings["Reporturl"] + "Export_EmptyVT.RPT");
                cryRpt.RecordSelectionFormula = "{VW_EXPORT_EMPTY_VT_PRINT_ASSGN.KUSRID} = '" + Session["CUSRID"].ToString() + "' AND {VW_EXPORT_EMPTY_VT_PRINT_ASSGN.VTDID} = " + id;
        ...
        //[Authorize(Roles = "ExportEmptyVTDelete")]
        public void Del()
        {

...
{
    public class ImportManualEInvoiceController : Controller
    {
        // GET: ImportManualEInvoice
        ...
        #region Index FORM
        [Authorize(Roles = "ImportManualEInvoiceIndex")]
        ...
        public JsonResult GetAjaxData(JQueryDataTableParamModel param)
        {
            using (var e = new CFSImportEntities())
            {
                ...
                var aaData = data.Select(d => new string[] { d.TRANDATE.Value.ToString("dd/MM/yyyy"), d.TRANTIME.Value.ToString("hh:mm tt"), d.TRANDNO.ToString(), d.TRANREFNAME, d.TRANNAMT.ToString(), d.DISPSTATUS, d.GSTAMT.ToString(), d.ACKNO, d.TRANMID.ToString() }).ToArray();
                return Json(new
                {
                    sEcho = param.sEcho,
                    aaData = aaData,
        ...
        [Authorize(Roles = "ImportManualEInvoiceUpload")]
        ...
        [Authorize(Roles = "ImportManualEInvoicePrint")]
        public void UPrintView(int? id = 0)
        {
            ...
            if (TMPRPT_IDS == "Successfully Added")
            {
                ...
                //if (rpttype == 0)
                cryRpt.Load(ConfigurationManager.AppSettings["Reporturl"] + "E_1003.RPT");
                //else cryRpt.Load(ConfigurationManager.AppSettings["Reporturl"] + "import_Invoice_group_rpt_E01.RPT");

                cryRpt.RecordSelectionFormula = "{VW_IMPORT_EINVOICE_MANUAL_BILL_DETAIL_RPT.KUSRID} = '" + Session["CUSRID"].ToString() + "' AND {VW_IMPORT_EINVOICE_MANUAL_BILL_DETAIL_RPT.TRANMID} = " + ids;

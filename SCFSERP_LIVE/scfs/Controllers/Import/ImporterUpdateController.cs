...
{
    [SessionExpire]
    public class ImporterUpdateController : Controller
    {
        // GET: ImporterUpdate
        ...
        #region AutoImporter
        public JsonResult AutoImporter(string term)/*model2.edmx*/
        {

            var result = (from r in context.categorymasters.Where(x => x.CATETID == 1 && x.DISPSTATUS == 0)
                    ...
                    try
                    {
                        ...
                        for (int count2 = 0; count2 < f_GIDID.Count(); count2++)
                        {
                            ...
                            if (boolSTFDIDS[count2] == "true")
                            {
                                ...
                                context.Database.ExecuteSqlCommand("UPDATE GATEINDETAIL SET IMPRTID=" + F_Form.Get("TRANREFID") + ",IMPRTNAME='" + F_Form.Get("TRANREFNAME") + "' WHERE GIDID=" + GIDID);
                            }

                        }

                        trans.Commit(); Response.Redirect("/ImporterUpdate/Form");
                    }
                    catch (Exception ex)
                    {
                        trans.Rollback();

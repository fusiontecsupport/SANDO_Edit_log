...
function onchangeautocomplete($objlabel, $objid) {
    var crntval = $($objlabel).val();
    if (crntval != "") {
        $obj.autocomplete({
...
function CheckVehicle($obj, $tranobj) {
    // alert($($obj).val());
    //alert($($tranobj).val());

    var sel_vhlno = $($obj).val();
    var tranid = $($tranobj).val();
    id = sel_vhlno + '~' + tranid;
    
    $.ajax({
        type: 'post',
        url: '/Common/ValidateDuplicateVehicle/' + id, // we are calling json method
        
        success: function (data) {
            
            if (data == "IN") {
                alert('Vehicle is already IN, Please check vehicle no!!');
                $($obj).val("");
                $($obj).focus();
                return false;
            }

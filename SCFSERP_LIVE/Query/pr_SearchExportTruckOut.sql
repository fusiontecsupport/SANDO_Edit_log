...
alter PROCEDURE [dbo].[pr_SearchExportTruckOut]  /* CHANGE */
...
WHERE (GATEOUTDETAIL.COMPYID= @LCOMPYID) AND (GODATE BETWEEN @LSDATE AND @LEDATE) AND (GATEOUTDETAIL.SDPTID = 2) AND (REGSTRID = 5) AND  (@FilterTerm IS NULL 
...
SELECT @TotalRowsCount = COUNT(*)
FROM   dbo.GATEOUTDETAIL           
WHERE (GATEOUTDETAIL.COMPYID= @LCOMPYID) AND (GODATE BETWEEN @LSDATE AND @LEDATE) AND (GATEOUTDETAIL.SDPTID = 2) AND (REGSTRID = 5)

...
CREATE PROCEDURE [dbo].[pr_SearchExportGateIn]  /* CHANGE */
...
AS BEGIN
    ...
    SELECT  GIDATE
      , GITIME
      , GINO 
      , CONTNRNO ,CONTNRSID  ,CHANAME,VHLNO
	 ,PRDTDESC
   ...
   FROM         dbo.GATEINDETAIL   /* CHANGE  TABLE NAME */
    WHERE (SDPTID=2) and (CONTNRID=1) and (COMPYID=@LCOMPYID) and  (GIDATE BETWEEN @LSDate AND @LEDate) AND (@FilterTerm IS NULL 
    ...
    SELECT GIDATE
      , GITIME
      , GINO 
      , CONTNRNO ,case CONTNRSID when 1 then 'ALL' when 2 then 'NOT REQUIRED' when 3 then '20'  when 4 then '40' when 5 then '45' end as  CONTNRSID,CHANAME ,VHLNO
	 ,PRDTDESC
   ...
   FROM        dbo.GATEINDETAIL  /* CHANGE  TABLE NAME */
    WHERE  (SDPTID=2) and (CONTNRID=1) and (COMPYID=@LCOMPYID) and (GIDATE BETWEEN @LSDate AND @LEDate)

alter PROCEDURE [dbo].[pr_Search_ExportManualBill]  /* CHANGE */
...
set @LTRANBTYPE=@PTRANBTYPE
    ...
    SELECT TRANDATE,TRANTIME, TRANDNO, TRANREFNAME, TRANNAMT, TRANMID, TRANCGSTAMT+TRANSGSTAMT+TRANIGSTAMT, DISPSTATUS,
			case when regstrid = 49 then 1 else 0 end,ACKNO
            , Row_Number() OVER (
            ORDER BY
...
WHERE   (COMPYID = @LCompyId) and (REGSTRID=@LREGSTRID) AND  (TRANBTYPE=@LTRANBTYPE or @LTRANBTYPE = 0) AND

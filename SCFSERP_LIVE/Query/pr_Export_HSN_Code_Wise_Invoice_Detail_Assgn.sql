USE [SCFS_ERP]
GO
/****** Object:  StoredProcedure [dbo].[pr_Export_HSN_Code_Wise_Invoice_Detail_Assgn]    Script Date: 25/12/2024 23:29:59 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

-- Exec pr_Export_HSN_Code_Wise_Invoice_Detail_Assgn 2, '01-apr-2023', '31-mar-2024'
alter PROCEDURE [dbo].[pr_Export_HSN_Code_Wise_Invoice_Detail_Assgn]
...
BEGIN
	...
	dbo.TRANSACTIONMASTER.TRANDNO, dbo.TRANSACTIONMASTER.TRANDATE, dbo.STUFFINGMASTER.STFBCHAGSTNO, 
	...
	INNER JOIN dbo.STUFFINGMASTER ON dbo.TRANSACTIONMASTER.TRANLMID = dbo.STUFFINGMASTER.STFMID 
    LEFT OUTER JOIN dbo.VW_TAXREGISTER_RPT_12 ON dbo.TRANSACTIONMASTER.TRANMID = dbo.VW_TAXREGISTER_RPT_12.TRANMID
    WHERE (dbo.TRANSACTIONMASTER.REGSTRID = 16) 
	...
	INNER JOIN dbo.STUFFINGMASTER ON dbo.TRANSACTIONMASTER.TRANLMID = dbo.STUFFINGMASTER.STFMID 
    LEFT OUTER JOIN dbo.VW_TAXREGISTER_RPT_12 ON dbo.TRANSACTIONMASTER.TRANMID = dbo.VW_TAXREGISTER_RPT_12.TRANMID
    WHERE (dbo.TRANSACTIONMASTER.REGSTRID = 17) 
    AND (dbo.TRANSACTIONMASTER.SDPTID = @PSDPTID)
    AND (dbo.TRANSACTIONMASTER.TRANDATE BETWEEN @PSDate AND @PEDate) 
    AND (dbo.TRANSACTIONMASTER.DISPSTATUS = 0)
    ORDER BY dbo.TRANSACTIONMASTER.TRANNO

	--Export Invoice Register id 49 - manual bill
	Insert into @TmpTable(TRANMID, REGSTRID, TRANNO, TRANDNO, TRANDATE, CATEAGSTNO, TRANREFNAME, STRG_HSNCODE, TRANSAMT, HANDL_HSNCODE,
	                      TRANDHAMT, TRANGAMT, CGSTAMT, SGSTAMT, IGSTAMT, TRANNAMT)
	...
	LEFT OUTER JOIN dbo.VW_TAXREGISTER_RPT_12 ON dbo.TRANSACTIONMASTER.TRANMID = dbo.VW_TAXREGISTER_RPT_12.TRANMID
    WHERE (dbo.TRANSACTIONMASTER.REGSTRID = 49) 
	AND (dbo.TRANSACTIONMASTER.TRANDATE BETWEEN @PSDate AND @PEDate) 

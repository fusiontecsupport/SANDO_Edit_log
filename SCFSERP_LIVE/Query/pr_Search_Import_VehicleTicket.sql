alter PROCEDURE [dbo].[pr_Search_Import_VehicleTicket]  /* CHANGE */  
...
set @LVTTYPE=@PVTTYPE  
 ...
 VTSSEALNO, GODID, VTTYPE , VTSTYPE, CHAID ,CHANAME,BOENO,IGMNO ,GPLNO, RowNum)  
 ...
 when cast(dbo.AUTHORIZATIONSLIPMASTER.ASLMTYPE as varchar(10)) = '2' then 'DeStuff' + (CASE VTSTYPE WHEN 1 THEN '(Full)' Else '(Part)' END) Else cast(dbo.AUTHORIZATIONSLIPMASTER.ASLMTYPE as varchar(10)) end,  
 VTSTYPE,
 dbo.GATEINDETAIL.CHAID, ISNULL(CATEGORYMASTER.CATENAME,'') AS CHANAME, dbo.GATEINDETAIL.BOENO,ISNULL(GATEINDETAIL.IGMNO,'') AS IGMNO,  
...
*/  
FROM         dbo.VEHICLETICKETDETAIL (nolock) INNER JOIN  
                      dbo.AUTHORIZATIONSLIPDETAIL (nolock)  ON  dbo.AUTHORIZATIONSLIPDETAIL.ASLDID=dbo.VEHICLETICKETDETAIL.ASLDID INNER JOIN  
        dbo.AUTHORIZATIONSLIPMASTER  (nolock) ON  dbo.AUTHORIZATIONSLIPMASTER.ASLMID= dbo.AUTHORIZATIONSLIPDETAIL.ASLMID INNER JOIN  
...
ORDER BY VTDNO   
 ...
 GIDId, ISNULL(CGIDID,0) AS CGIDID,  VTSSEALNO, isnull(GODID ,0) as GODID, case when isnull(GODID ,0) =0 then 'Not-Exists' else 'Exists' end 'GOSTS', VTSTYPE  
    FROM    @TableMaster  

-- select * from VW_STUFFING_ORGSBILLNO_REFNO_CBX_ASSGN (nolock) where SBMID=23447  AND (KUSRID='ck' OR KUSRID = '') order by SBMID,SBDID
-- select * from SHIPPINGBILLMASTER where SBMID=23447
-- select * from SHIPPINGBILLDETAIL where SBMID=23447
-- select * from VW_YEAR_DETAIL_CBX_ASSGN where 
/*
select TOP (100) PERCENT dbo.SHIPPINGBILLMASTER.CHAID
...
WHERE       
*/
alter VIEW [dbo].[VW_STUFFING_ORGSBILLNO_REFNO_CBX_ASSGN]     
AS    
SELECT        TOP (100) PERCENT dbo.SHIPPINGBILLMASTER.CHAID, dbo.SHIPPINGBILLDETAIL.SBMID, dbo.SHIPPINGBILLDETAIL.SBDID, LTRIM(dbo.SHIPPINGBILLMASTER.SBMDNO) 

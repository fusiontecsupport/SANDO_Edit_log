USE [SCFS_ERP]
GO
/****** Object:  StoredProcedure [dbo].[pr_SearchEmptyVT]    Script Date: 20/10/2021 18:42:57 ******/
SET ANSI_NULLS ON
GO
...
EXEC [dbo].[pr_Search_Empty_VT] NULL, 1, 'ASC', 1, 10, @TotalRowsCount= @total OUTPUT,@FilteredRowsCount= @filtered OUTPUT

SELECT @total, @filtered
*/

create PROCEDURE [dbo].[pr_Search_Empty_VT]  /* CHANGE */
...
GOTIME ,GONO,GODNO,CONTNRNO ,CONTNRSCODE,VHLNO,CHANAME ,GDRVNAME,IGMNO,GPLNO,VSLNAME,STMRNAME,PRDTDESC,GIDID,GODID, RowNum)
    SELECT  GODATE
            , GOTIME
			,GONO
			,GODNO
...
WHERE (GATEOUTDETAIL.COMPYID= @LCOMPYID) AND (GODATE BETWEEN @LSDATE AND @LEDATE) AND (GATEINDETAIL.SDPTID = 3) AND (REGSTRID = 1) AND  (@FilterTerm IS NULL 
...
FROM     dbo.GATEINDETAIL INNER JOIN
                      dbo.CONTAINERSIZEMASTER ON dbo.GATEINDETAIL.CONTNRSID = dbo.CONTAINERSIZEMASTER.CONTNRSID  LEFT OUTER JOIN
                      dbo.GATEOUTDETAIL ON dbo.GATEINDETAIL.GIDID = dbo.GATEOUTDETAIL.GIDID         
WHERE (GATEOUTDETAIL.COMPYID= @LCOMPYID) AND (GODATE BETWEEN @LSDATE AND @LEDATE) AND (GATEINDETAIL.SDPTID = 3) AND (REGSTRID = 1)

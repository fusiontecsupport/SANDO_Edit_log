...
EXEC [dbo].[pr_Search_Bond_Information] NULL, 1, 'ASC', 1, 10, @total OUTPUT, @filtered OUTPUT, 
...
alter PROCEDURE [dbo].[pr_Search_Bond_Information]  /* CHANGE */
...
AS BEGIN
    ...
    INSERT INTO @TableMaster(BNDDATE , BNDDNO , BNDNO ,  BNDCIFAMT   , CHANAME  ,IMPRTRNAME ,DISPSTATUS, 
	BNDID, RowNum)
  
    SELECT  Convert(varchar,BNDDATE,103), BNDDNO, BNDNO , BNDCIFAMT  ,  B.CATENAME, C.CATENAME , A.DISPSTATUS, A.BNDID
   ...
   FROM         dbo.BONDMASTER A (nolock)  
    ...
    WHERE   (BNDDATE BETWEEN @PSDate AND @PEDate) and (COMPYID=@LCompyid) AND SDPTID=10
	AND (@FilterTerm IS NULL 
              OR BNDDATE LIKE @FilterTerm
    ...
    SELECT BNDDATE, BNDDNO, BNDNO , BNDCIFAMT  ,  CHANAME, IMPRTRNAME, --DISPSTATUS, BNDID,

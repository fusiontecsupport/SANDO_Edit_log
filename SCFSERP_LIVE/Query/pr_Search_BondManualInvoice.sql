alter PROCEDURE [dbo].[pr_Search_BondManualInvoice]  /* CHANGE */
...
FROM       BONDTRANSACTIONMASTER (NOLOCK)
WHERE (dbo.BONDTRANSACTIONMASTER.COMPYID = @LCompyId) and TRANTID = 4 AND
taxtype = @PTAXTYPE and
(TRANDATE BETWEEN @LSDate AND @LEDate)
--and (BONDTRANSACTIONMASTER.REGSTRID=@LREGSTRID) 
--and (BONDTRANSACTIONMASTER.TRANBTYPE IN(1, 2, 3, 4, 5)) and
...
WHERE (dbo.BONDTRANSACTIONMASTER.COMPYID = @LCompyId) and TRANTID = 4 AND
taxtype = @PTAXTYPE and
--(BONDTRANSACTIONMASTER.REGSTRID=@LREGSTRID) and (BONDTRANSACTIONMASTER.TRANBTYPE IN(1, 2, 3, 4, 5)) and
(TRANDATE BETWEEN @LSDate AND @LEDate) --AND (BONDTRANSACTIONMASTER.SDPTID=1)

ALTER PROC PR_IMPORT_MANUAL_BILL_TRANMASTER_UPD
@PTRANMID INT
AS
BEGIN
	...
	FROM TRANSACTIONMASTER  B(NOLOCK) 
			...
			WHERE  REGSTRID = 15 AND (TRAND_HANDL_CGST_AMT > 0  OR TRAND_HANDL_IGST_AMT > 0) AND XA.TRANMID =  @LTRANMID
	...
	FROM TRANSACTIONMASTER  B(NOLOCK) 
			...
			FROM TRANSACTIONDETAIL XA(NOLOCK) JOIN TRANSACTIONMASTER  XB(NOLOCK) ON XA.TRANMID = XB.TRANMID
			WHERE  REGSTRID = 15 AND (TRAND_HANDL_CGST_AMT > 0  OR TRAND_HANDL_CGST_EXPRN > 0) AND XA.TRANMID =  @LTRANMID
			GROUP BY XA.TRANMID) A
			ON A.TRANMID = B.TRANMID 
	WHERE REGSTRID = 15
	AND HANDL_HSNCODE <> ''
